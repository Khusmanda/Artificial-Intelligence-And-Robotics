...
class MoveHokuyo():
    ...
    def __init__(self):
        ...
        # Init Publisher
        self.Hokuyo_vel_publisher = rospy.Publisher(
    ...
    def stop_Hokuyo(self):
        ...
        while i < 20:
            self.Hokuyo_vel_publisher.publish(self.cmd)
            self.rate.sleep()
            i += 1

    def move_forward(self, linear_speed=0.8, angular_speed=0.0):
...
if __name__ == '__main__':
    time.sleep(1)
    print("Slept for 1 sec for particle dispersion visibility")
    rospy.init_node('move_Hokuyo_node', anonymous=True)
    MoveHokuyo_object = MoveHokuyo()

    cov = 1

    while cov > 0.65:
        MoveHokuyo_object.call_service()
        MoveHokuyo_object.move_square()
        cov = MoveHokuyo_object.calculate_covariance()
